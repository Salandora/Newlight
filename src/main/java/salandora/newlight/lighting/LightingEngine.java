...
import salandora.newlight.mixin.EnumFacingMixin;
...
public class LightingEngine {
    ...
    private final PooledLongQueue[] queuedLightUpdates = new PooledLongQueue[EnumLightType.values().length];
    ...
    private EnumLightType lightType;
    ...
    public LightingEngine(final World world)
    {
        ...
        for (int i = 0; i < EnumLightType.values().length; ++i)
        {
    ...
    /**
     * Schedules a light update for the specified light type and position to be processed later by {@link #procLightUpdates(EnumLightType)}
     */
     ...
     */
    private void scheduleLightUpdate(final EnumLightType lightType, final long pos)
    {
    ...
    /**
     * Calls {@link #procLightUpdates(EnumLightType)} for both light types
     */
    public void procLightUpdates()
    {
        this.procLightUpdates(EnumLightType.SKY);
        this.procLightUpdates(EnumLightType.BLOCK);
    }

    /**
     * Processes light updates of the given light type
     */
    public void procLightUpdates(final EnumLightType lightType)
    {
        final PooledLongQueue queue = this.queuedLightUpdates[lightType.ordinal()];
        ...
        if (this.world.isRemote && !Minecraft.getInstance().isCallingFromMinecraftThread())
        {
            return;
        }
    ...
    {
        return chunk.getLight(this.lightType, pos, this.world.dimension.hasSkyLight());
    }
    ...
    {
        if (this.lightType == EnumLightType.SKY)
        {
            return this.curChunk.canSeeSky(this.curPos) ? EnumLightType.SKY.defaultLightValue : 0;
        }

        return MathHelper.clamp(LightUtils.getLightValue(state, this.world, this.curPos), 0, MAX_LIGHT);
    }
    ...
    {
        return MathHelper.clamp(state.getOpacity(this.world, pos), 1, MAX_LIGHT);
    }
    ...
    {
        return this.world.getChunkProvider().getChunk(pos.getX() >> 4, pos.getZ() >> 4, false, false);
    }
