...
public class LightInitHooks
{
    ...
    {
        if (((IChunkNewLight)chunk).getIsLightPopulated() || ((IChunkNewLight)chunk).getPendingNeighborLightInits() != 0)
            return;

        ((IChunkNewLight)chunk).setPendingNeighborLightInits((short)31);
    ...
    static void initNeighborLight(final World world, final Chunk chunk, final Chunk nChunk, final EnumFacing nDir)
    {
        ...
        if ((((IChunkNewLight)chunk).getPendingNeighborLightInits() & flag) == 0)
            return;

        ((IChunkNewLight)chunk).setPendingNeighborLightInits((short)(((IChunkNewLight)chunk).getPendingNeighborLightInits() ^ flag));
    ...
    {
        if (((IChunkNewLight)chunk).getPendingNeighborLightInits() != 16)
            return;
        ...
        ((IChunkNewLight)chunk).setPendingNeighborLightInits((short)0);
        ((IChunkNewLight)chunk).setIsLightPopulated(true);
        chunk.markDirty();
    }

    public static void writeNeighborInitsToNBT(final Chunk chunk, final NBTTagCompound nbt)
    {
        if (((IChunkNewLight) chunk).getPendingNeighborLightInits() != 0)
            nbt.putShort(neighborLightInitsKey, ((IChunkNewLight) chunk).getPendingNeighborLightInits());
    }
    ...
    public static void readNeighborInitsFromNBT(final Chunk chunk, final NBTTagCompound nbt)
    {
        if (nbt.contains(neighborLightInitsKey, 2))
            ((IChunkNewLight) chunk).setPendingNeighborLightInits(nbt.getShort(neighborLightInitsKey));
    }
}
