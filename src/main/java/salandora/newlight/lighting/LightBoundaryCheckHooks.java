...
public class LightBoundaryCheckHooks
{
    ...
    private static void flagChunkBoundaryForUpdate(final Chunk chunk, final int index, final int sectionMask, final EnumLightType lightType)
    {
        ...
        ((IChunkNewLight)chunk).getNeighborLightChecks()[index] |= sectionMask << (LightUtils.getIndex(lightType) << 4);
        chunk.markDirty();
    }
...
//            ((IChunkNewLight)chunk).setPendingBoundaryChecks(true);
    ...
    {
        if (((IChunkNewLight)neighborChunk).getNeighborLightChecks() == null)
            return;
        ...
        {
            final int neighborFlags = ((IChunkNewLight)neighborChunk).getNeighborLightChecks()[outIndex + offset];

            if (neighborFlags != 0)
            {
                initNeighborLightChecks(chunk);
                ((IChunkNewLight)chunk).getNeighborLightChecks()[(inIndex + offset) & 7] |= neighborFlags;
                ((IChunkNewLight)neighborChunk).getNeighborLightChecks()[outIndex + offset] = 0;

                LightTrackingHooks.initLightTrackings(neighborChunk);
                ((IChunkNewLight)neighborChunk).getLightTrackings()[LightTrackingHooks.getHorizontalFlagIndex(oppDir) + offset] |= neighborFlags;
            }
        }
    ...
    static void scheduleRelightChecksForChunkBoundariesClient(final World world, final Chunk chunk)
    {
        ...
        for (int i = 0; i <= 1; ++i)
        {
            final int flags = ((IChunkNewLight)chunk).getNeighborLightChecks()[LightBoundaryCheckHooks.OUT_INDEX_OFFSET + i];

            if (flags != 0)
            {
                ((IChunkNewLight)chunk).getNeighborLightChecks()[LightBoundaryCheckHooks.OUT_INDEX_OFFSET] = 0;
    ...
    )
    {
        ...
        final int flags = ((IChunkNewLight)chunk).getNeighborLightChecks()[flagIndex];

        if (flags == 0)
            return;

        ((IChunkNewLight)chunk).getNeighborLightChecks()[flagIndex] = 0;
    ...
    {
        initNeighborLightChecks(chunk);
        ((IChunkNewLight)chunk).getNeighborLightChecks()[index] |= sectionMask;
        ((IChunkNewLight)chunk).setPendingBoundaryChecks(true);
    }
    ...
    {
        if (((IChunkNewLight)chunk).getNeighborLightChecks() == null)
            ((IChunkNewLight)chunk).setNeighborLightChecks(new int[chunk.getWorld().isRemote ? FLAG_COUNT_CLIENT : FLAG_COUNT_SERVER]);
    }
    ...
    static void writeNeighborLightChecksToNBT(final Chunk chunk, final NBTTagCompound nbt)
    {
        ...
        for (final int flags : ((IChunkNewLight)chunk).getNeighborLightChecks())
        {
        ...
        if (empty)
            ((IChunkNewLight)chunk).setNeighborLightChecks(null);
        else
        ...
        if (nbt.contains(neighborLightChecksKey, 9))
        {
            ...
            if (list.size() == FLAG_COUNT_SERVER)
            {
                ...
                for (int i = 0; i < FLAG_COUNT_SERVER; ++i)
                    ((IChunkNewLight)chunk).getNeighborLightChecks()[i] = (short)((NBTTagInt) list.get(i)).getInt();
            }
            else
    ...
    public static void onLoad(final World world, final Chunk chunk)
    {
        ...
        for (int x = -1; x <= 1; ++x)
            for (int z = -1; z <= 1; ++z)
            {
                ...
                if (nChunk != null)
                {
                    ((IChunkNewLight)chunk).setNeighborsLoaded(((IChunkNewLight)chunk).getNeighborsLoaded() + 1);
                    ((IChunkNewLight)nChunk).setNeighborsLoaded(((IChunkNewLight)nChunk).getNeighborsLoaded() + 1);
                }
            }
    }
    ...
    public static void onTick(final World world, final Chunk chunk)
    {
        ...
        if (((IChunkNewLight)chunk).getPendingBoundaryChecks() && ((IChunkNewLight)chunk).getNeighborsLoaded() == 8)
        {
            scheduleRelightChecksForChunkBoundariesClient(world, chunk);
            ((IChunkNewLight)chunk).setPendingBoundaryChecks(false);
        }
    }
